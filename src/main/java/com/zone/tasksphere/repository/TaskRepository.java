...
public interface TaskRepository extends JpaRepository<Task, UUID>, JpaSpecificationExecutor<Task> {
        ...
        @Query("""
                            ...
                            WHERE t.sprint.id = :sprintId
                              ...
                              AND t.deletedAt IS NULL
                        """)
        List<Task> findTasksBySprintAndStatus(
                        @Param("sprintId") UUID sprintId,
        ...
        @Query(value = "SELECT MAX(CAST(SUBSTRING(task_code, CHAR_LENGTH(:projectKey) + 2) AS UNSIGNED)) " +
                        "FROM tasks WHERE project_id = :projectId AND task_code LIKE CONCAT(:projectKey, '-%')", nativeQuery = true)
        Optional<Integer> findMaxTaskSequence(@Param("projectId") UUID projectId,
        ...
        /**
         * BR-24: Soft delete đệ quy — cập nhật trực tiếp qua JPQL UPDATE (bỏ
         * qua @SQLRestriction)
         */
        @Modifying
        @Query("""
        ...
        /**
         * Lấy các task instance chưa bắt đầu và chưa gán sprint (dùng để soft-delete
         * khi hủy recurrence)
         */
        @Query("""
                            SELECT t FROM Task t
         ...
         * Trả về [total, done, todo, inProgress, inReview, cancelled, overdue, totalSp,
         * doneSp,
