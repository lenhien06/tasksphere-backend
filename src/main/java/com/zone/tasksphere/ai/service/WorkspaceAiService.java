...
public class WorkspaceAiService {
    ...
    public AnalyzeDescriptionResponse analyzeDescription(UUID wsId,
        ...
        AnalyzeDescriptionResponse.UnderstoodFields understood = AnalyzeDescriptionResponse.UnderstoodFields.builder()
    ...
    public GenerateProjectPlanResponse generateProjectPlan(UUID wsId,
        ...
        try {
            GenerateProjectPlanResponse response = objectMapper.readValue(raw, GenerateProjectPlanResponse.class);
            ...
            if (response.getSprints() != null) {
                ...
                int totalSP = response.getSprints().stream()
                        ...
                        .mapToInt(GenerateProjectPlanResponse.TaskPlanDto::getStoryPoints).sum();
    ...
    public ConfirmProjectPlanResponse confirmProjectPlan(UUID wsId,
        ...
        columns.add(ProjectStatusColumn.builder().project(project).name("To Do")
                .colorHex("#D9D9D9").sortOrder(1).isDefault(true).mappedStatus(TaskStatus.TODO).build());
        ...
        columns.add(ProjectStatusColumn.builder().project(project).name("Done")
                .colorHex("#52C41A").sortOrder(4).mappedStatus(TaskStatus.DONE).build());
        ...
        if (plan.getMemberIds() != null) {
            for (String memberId : plan.getMemberIds()) {
                try {
                    ...
                    if (uid.equals(creatorId)) continue; // creator already added as PM
        ...
        for (GenerateProjectPlanResponse.SprintPlanDto sprintDto : sprintDtos) {
            ...
            for (GenerateProjectPlanResponse.TaskPlanDto taskDto : taskDtos) {
                ...
                if (taskDto.getSuggestedAssigneeId() != null && !taskDto.getSuggestedAssigneeId().isBlank()) {
                    ...
                    } catch (IllegalArgumentException ignored) { /* LLM returned bad UUID */ }
                }
                ...
                if (assignee != null) {
                    ...
                    projectMemberRepository.findByProjectIdAndUserId(saved.getId(), assignee.getId())
                            .ifPresent(pm -> {
        ...
        return AnalyzeDescriptionResponse.AiQuestion.builder()
                ...
                .options(List.of(
                        AnalyzeDescriptionResponse.QuestionOption.builder()
                                .label("React + Node.js + MongoDB").value("React,Node.js,MongoDB").build(),
                        AnalyzeDescriptionResponse.QuestionOption.builder()
                                .label("React + Java Spring Boot + MySQL").value("React,Java,Spring Boot,MySQL").build(),
                        AnalyzeDescriptionResponse.QuestionOption.builder()
                                .label("Vue.js + Python Django").value("Vue.js,Python,Django").build(),
                        AnalyzeDescriptionResponse.QuestionOption.builder()
                                .label("Angular + .NET + SQL Server").value("Angular,.NET,SQL Server").build()
                ))
                .allowCustom(true)
    ...
    private AnalyzeDescriptionResponse.AiQuestion buildDeadlineQuestion() {
        return AnalyzeDescriptionResponse.AiQuestion.builder()
                ...
                .options(List.of(
                        AnalyzeDescriptionResponse.QuestionOption.builder().label("2 tuần").value("2").build(),
                        AnalyzeDescriptionResponse.QuestionOption.builder().label("1 tháng").value("4").build(),
                        AnalyzeDescriptionResponse.QuestionOption.builder().label("6 tuần").value("6").build(),
                        AnalyzeDescriptionResponse.QuestionOption.builder().label("3 tháng+").value("12").build()
                ))
                .allowCustom(true)
        ...
        List<AnalyzeDescriptionResponse.MemberOption> memberOptions = wms.stream()
                .map(wm -> AnalyzeDescriptionResponse.MemberOption.builder()
                        ...
                        .skillTags(wm.getSkillTags() != null ? wm.getSkillTags()
                                : (wm.getUser().getSkillTags() != null ? wm.getUser().getSkillTags() : List.of()))
                        .build())
    ...
    private String buildMembersContext(UUID wsId, List<String> memberIds) {
        if (memberIds == null || memberIds.isEmpty()) return "(chưa chọn thành viên)";
    ...
    private String resolveUniqueProjectKey(String base) {
        if (base == null || base.isBlank()) base = "PROJ";
        String key = base.toUpperCase().replaceAll("[^A-Z0-9]", "").substring(0, Math.min(base.length(), 6));
        if (key.isBlank()) key = "PROJ";
        if (!projectRepository.existsByProjectKey(key)) return key;
        int suffix = 2;
        while (projectRepository.existsByProjectKey(key + suffix)) suffix++;
        return key + suffix;
    }
