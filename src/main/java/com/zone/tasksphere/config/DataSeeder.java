...
/**
 * Development data seeder — idempotent, runs on every startup except in test
 * profile.
...
public class DataSeeder implements CommandLineRunner {
        ...
        @Transactional
        void seedAdminUser() {
                if (userRepository.existsByEmail("admin@tasksphere.local"))
                        return;
        ...
        void seedDemoProject() {
                if (projectRepository.findByKeyWithDeleted("DEMO").isPresent())
                        return;
        ...
        void seedEcomProject() {
                if (projectRepository.findByKeyWithDeleted("ECOM").isPresent())
                        return;
                User admin = u("admin@tasksphere.local");
                User pm = u("pm@tasksphere.local");
                User dev1 = u("dev1@tasksphere.local");
        ...
        void seedMobileProject() {
                if (projectRepository.findByKeyWithDeleted("MOBILE").isPresent())
                        return;
                User admin = u("admin@tasksphere.local");
                User pm = u("pm@tasksphere.local");
                User dev2 = u("dev2@tasksphere.local");
        ...
        void seedProjectStatusColumns() {
                for (String key : List.of("DEMO", "ECOM", "MOBILE")) {
                        ...
                        if (p == null)
                                continue;
                        if (projectStatusColumnRepository.existsByProjectAndName(p, "To Do"))
                                continue;
                        ...
                        projectStatusColumnRepository
                                        .save(col(p, "In Review", "#FAAD14", 3, TaskStatus.IN_REVIEW));
        ...
        private void seedSprintsFor(String projectKey, S... defs) {
                ...
                if (p == null)
                        return;
                if (!sprintRepository.findByProjectOrderByStartDateAsc(p).isEmpty())
                        return;
                for (S d : defs) {
        ...
        @Transactional
        void seedDemoTasks() {
                if (taskRepository.findByTaskCode("DEMO-001").isPresent())
                        return;
                ...
                ProjectStatusColumn inReview = cols.get(2);
                ...
                t("DEMO-001", p, s1, done, TaskStatus.DONE, "Setup Spring Boot project structure",
                                ...
                                "Khởi tạo project Spring Boot 3.5.5, cấu hình Maven wrapper, Lombok, Spring Security placeholder.",
                                1);
                ...
                t("DEMO-002", p, s1, done, TaskStatus.DONE, "Thiết kế ERD & Database schema",
                                ...
                                "Phân tích business requirements và thiết kế toàn bộ ERD 28 tables cho hệ thống TaskSphere.",
                                2);
                ...
                t("DEMO-005", p, s1, done, TaskStatus.DONE, "JWT Authentication — Login & Signup",
                                ...
                                "Login/signup với JWT access token (15m) + refresh token (7d). BCrypt pw strength=12.",
                                5);
                ...
                t("DEMO-015", p, s2, done, TaskStatus.DONE, "UI/UX Design — Wireframe toàn hệ thống",
                                ...
                                "Wireframe tất cả màn hình: Dashboard, Kanban, Task Detail, Sprint board, Settings.",
                                7);
                ...
                Task epic1 = taskRepository.save(task("DEMO-017", p, s3, todo, TaskStatus.TODO,
                                ...
                                "Hệ thống thông báo real-time cho toàn bộ sự kiện: task update, mentions, sprint events.",
                                1));
                sub("DEMO-018", p, todo, "WebSocket Server Setup (STOMP/SockJS)", TaskPriority.HIGH, dev3, admin,
                                epic1);
                sub("DEMO-019", p, inProg, "Notification Service & Event Publisher", TaskPriority.HIGH, dev3, admin,
                                epic1);
                sub("DEMO-020", p, todo, "Frontend — WebSocket Client Integration", TaskPriority.MEDIUM, dev4, admin,
                                epic1);
                ...
                sub("DEMO-023", p, inProg, "@Mention parsing & Notification trigger", TaskPriority.MEDIUM, dev1, pm,
                                epic2);
                ...
                t("DEMO-026", p, s3, inReview, TaskStatus.IN_PROGRESS, "Worklog — Time Tracking API",
                ...
                t("DEMO-029", p, s3, inProg, TaskStatus.IN_PROGRESS, "Fix: Refresh token không invalidate sau logout",
                                ...
                                "Sau logout, refresh token cũ vẫn dùng được để lấy access token mới. Security issue!",
                                7);
                ...
                t("DEMO-030", p, s3, todo, TaskStatus.TODO, "Perf: N+1 query trong Task list API",
                                ...
                                "TaskRepository trigger N+1 khi load assignee/reporter. Cần @EntityGraph hoặc JOIN FETCH.",
                                8);
        ...
        @Transactional
        void seedEcomTasks() {
                if (taskRepository.findByTaskCode("ECOM-001").isPresent())
                        return;
                ...
                t("ECOM-014", p, s2, inReview, TaskStatus.IN_PROGRESS, "Product Search — Elasticsearch",
                                TaskType.TASK, TaskPriority.HIGH, dev3, pm, 8,
        ...
        void seedMobileTasks() {
                if (taskRepository.findByTaskCode("MOBILE-001").isPresent())
                        return;
        ...
        private void assignVersionIfMissing(String taskCode, ProjectVersion version) {
                ...
                if (task == null || task.getProjectVersion() != null)
                        return;
        ...
        private void addDependency(String blockingCode, String blockedCode, DependencyType type, String creatorEmail) {
                ...
                if (blocking == null || blocked == null)
                        return;
                if (taskDependencyRepository.existsByBlockingTaskIdAndBlockedTaskId(blocking.getId(), blocked.getId()))
                        return;
        ...
        void seedCustomFieldsAndValues() {
                ...
                CustomField effort = ensureCustomField(demo, "Estimated Effort (days)", CustomFieldType.NUMBER, false,
                                false, 2,
                                null);
                CustomField customerVisible = ensureCustomField(demo, "Customer Visible", CustomFieldType.BOOLEAN,
                                false, false, 3,
                                null);
                ...
                upsertCustomValue("DEMO-028", releaseNote, "Fix drag/drop position race condition for Kanban board.",
                                null, null, null);
        ...
        private CustomField ensureCustomField(Project project, String name, CustomFieldType type, boolean required,
                ...
                if (customFieldRepository.existsByProjectIdAndNameAndDeletedAtIsNull(project.getId(), name)) {
                        return customFieldRepository
                                        .findByProjectIdAndDeletedAtIsNullOrderBySortOrderAsc(project.getId())
                                        .stream()
        ...
        private void upsertCustomValue(String taskCode, CustomField field,
                ...
                if (task == null)
                        return;
                if (customFieldValueRepository.findByTaskIdAndCustomFieldId(task.getId(), field.getId()).isPresent())
                        return;
        ...
        void seedSavedFilters() {
                ...
                if (savedFilterRepository.findByProjectIdAndCreatedByIdOrderByCreatedAtDesc(demo.getId(), pm.getId())
                                .isEmpty()) {
                ...
                if (savedFilterRepository.findByProjectIdAndCreatedByIdOrderByCreatedAtDesc(ecom.getId(), qa.getId())
                                .isEmpty()) {
                        savedFilterRepository.save(SavedFilter.builder()
                                        .project(ecom)
                                        .createdBy(qa)
        ...
        void seedProjectInvites() {
                ...
                inviteIfMissing("seed-ecom-pending-viewer", ecom, pm, "new.viewer@tasksphere.local",
                                ProjectRole.VIEWER, InviteStatus.PENDING, Instant.now().plusSeconds(5 * 24 * 3600),
                                null, null);
                inviteIfMissing("seed-ecom-accepted-dev2", ecom, pm, dev2.getEmail(),
                                ProjectRole.MEMBER, InviteStatus.ACCEPTED, Instant.now().plusSeconds(7 * 24 * 3600),
                                Instant.now().minusSeconds(2 * 24 * 3600), dev2);
                inviteIfMissing("seed-ecom-expired-qa", ecom, pm, "expired.qa@tasksphere.local",
                                ProjectRole.MEMBER, InviteStatus.EXPIRED, Instant.now().minusSeconds(2 * 24 * 3600),
                                null, null);
                inviteIfMissing("seed-ecom-revoked-ba", ecom, pm, "revoked.ba@tasksphere.local",
                                ProjectRole.MEMBER, InviteStatus.REVOKED, Instant.now().plusSeconds(2 * 24 * 3600),
                                null, null);
        }
        ...
        private void inviteIfMissing(String token, Project project, User inviter, String inviteeEmail,
                ...
                if (projectInviteRepository.findByToken(token).isPresent())
                        return;
        ...
        @Transactional
        void seedUploadJobs() {
                if (uploadJobRepository.findAll().stream()
                                .anyMatch(j -> "seed-evidence.zip".equals(j.getOriginalFileName()))) {
                        return;
                }
                Task t028 = taskRepository.findByTaskCode("DEMO-028").orElse(null);
                Task t025 = taskRepository.findByTaskCode("DEMO-025").orElse(null);
        ...
        void seedComments() {
                ...
                if (t002 == null)
                        return;
                if (!commentRepository.findByTaskAndParentCommentIsNullOrderByCreatedAtAsc(t002).isEmpty())
                        return;
                ...
                Comment c1 = commentRepository.save(Comment.builder().task(t002).author(ba)
                                .content("<p>Đã hoàn thành bản ERD v1 — <strong>28 tables</strong> với đầy đủ relationships. @pm anh review và confirm trước khi team bắt đầu implement nhé!</p>")
                                .build());
                ...
                if (t011 != null) {
                        Comment c2 = commentRepository.save(Comment.builder().task(t011).author(pm)
                                        .content("<p>Lưu ý implement <strong>Optimistic Locking</strong> dùng <code>@Version</code> để tránh conflict khi nhiều user cùng edit task cùng lúc.</p>")
                                        .build());
                        ...
                        commentRepository.save(Comment.builder().task(t011).author(qa)
                                        .content("<p>Test concurrent update đã pass ✓. Thêm test case cho 3+ users cùng update để chắc chắn hơn.</p>")
                                        .build());
                }
                ...
                if (t028 != null) {
                        commentRepository.save(Comment.builder().task(t028).author(qa)
                                        .content("<p><strong>Steps to reproduce:</strong><br/>" +
                                                        ...
                                                        "<em>Repro rate: 100%</em></p>")
                                        .build());
                        Comment c3 = commentRepository.save(Comment.builder().task(t028).author(dev1)
                                        .content("<p><strong>Root cause:</strong> <code>KanbanService.moveTask()</code> query MAX position trước khi insert nhưng không lock row → race condition khi nhiều user drag cùng lúc.</p>")
                                        .build());
                ...
                if (t029 != null) {
                        commentRepository.save(Comment.builder().task(t029).author(dev3)
                                        .content("<p><strong>Fix plan:</strong> Logout → set <code>RefreshToken.revokedAt = now()</code> + add token vào Redis blacklist (TTL = expire time).</p>")
                                        .build());
                        commentRepository.save(Comment.builder().task(t029).author(admin)
                                        .content("<p>Good approach! Nhớ handle edge case khi Redis down — fallback về DB check để tránh allow revoked token.</p>")
                                        .build());
                }
                ...
                if (e016 != null) {
                        commentRepository.save(Comment.builder().task(e016).author(qa)
                                        .content("<p><strong>Bug confirmed:</strong> Giá 100k → hiển thị 110k (đúng) nhưng cart tính 121k (110k + 10% VAT lần 2). Ảnh hưởng <em>tất cả</em> sản phẩm có VAT.</p>")
                                        .build());
                        commentRepository.save(Comment.builder().task(e016).author(dev3)
                                        .content("<p><strong>Found it:</strong> <code>ProductDTO.getEffectivePrice()</code> apply VAT 2 lần — một lần ở service layer, một lần ở frontend <code>priceFormatter</code>.</p>")
                                        .build());
                }
                ...
                if (m007 != null) {
                        commentRepository.save(Comment.builder().task(m007).author(qa)
                                        .content("<p>Repro: Android 13, Samsung Galaxy S23. Bấm vào input cuối form → keyboard hiện → che khuất input, không tự scroll lên.</p>")
                                        .build());
                        commentRepository.save(Comment.builder().task(m007).author(dev4)
                                        .content("<p>Cần wrap bằng <code>KeyboardAwareScrollView</code> từ thư viện <code>react-native-keyboard-aware-scroll-view</code> thay vì dùng native <code>KeyboardAvoidingView</code>.</p>")
                                        .build());
                }
        }
        ...
        void seedWorklogs() {
                ...
                if (t005 == null)
                        return;
                if (!worklogRepository.findByTaskId(t005.getId()).isEmpty())
                        return;
                ...
                if (t013 != null) {
                        ...
                        wl(t013, dev4, LocalDate.now().minusDays(35), 10800,
                                        "Implement moveTask với position recalculation");
                ...
                if (t015 != null) {
                        ...
                        wl(t015, designer, LocalDate.now().minusDays(40), 21600,
                                        "Wireframe Task Detail & Sprint board");
                ...
                if (e014 != null) {
                        wl(e014, dev3, LocalDate.now().minusDays(10), 14400,
                                        "Setup Elasticsearch, index product mapping");
        ...
        void seedActivityLogs() {
                ...
                if (demo == null)
                        return;
                if (activityLogRepository.findByProjectId(demo.getId(), Pageable.ofSize(1)).hasContent())
                        return;
                ...
                log(admin, EntityType.PROJECT, demoId, ActionType.CREATED, null,
                                "{\"name\":\"TaskSphere Platform\",\"key\":\"DEMO\"}", demoId);
                log(ba, EntityType.TASK, t001.getId(), ActionType.CREATED, null, "{\"taskCode\":\"DEMO-001\"}", demoId);
                log(dev3, EntityType.TASK, t005.getId(), ActionType.STATUS_CHANGED, "{\"status\":\"TODO\"}",
                                "{\"status\":\"IN_PROGRESS\"}", demoId);
                log(dev3, EntityType.TASK, t005.getId(), ActionType.STATUS_CHANGED, "{\"status\":\"IN_PROGRESS\"}",
                                "{\"status\":\"DONE\"}", demoId);
                log(dev1, EntityType.TASK, t011.getId(), ActionType.ASSIGNED, null,
                                "{\"assignee\":\"Trần Lập Trình\",\"taskCode\":\"DEMO-011\"}", demoId);
                log(pm, EntityType.TASK, t011.getId(), ActionType.UPDATED, "{\"storyPoints\":8}",
                                "{\"storyPoints\":13}", demoId);
                log(dev1, EntityType.TASK, t011.getId(), ActionType.STATUS_CHANGED, "{\"status\":\"TODO\"}",
                                "{\"status\":\"IN_PROGRESS\"}", demoId);
                log(dev1, EntityType.TASK, t011.getId(), ActionType.STATUS_CHANGED, "{\"status\":\"IN_PROGRESS\"}",
                                "{\"status\":\"DONE\"}", demoId);
                log(qa, EntityType.TASK, t028.getId(), ActionType.CREATED, null,
                                "{\"taskCode\":\"DEMO-028\",\"type\":\"BUG\",\"priority\":\"CRITICAL\"}", demoId);
                log(dev1, EntityType.TASK, t028.getId(), ActionType.STATUS_CHANGED, "{\"status\":\"TODO\"}",
                                "{\"status\":\"IN_PROGRESS\"}", demoId);
                log(qa, EntityType.TASK, t029.getId(), ActionType.CREATED, null,
                                "{\"taskCode\":\"DEMO-029\",\"type\":\"BUG\",\"priority\":\"HIGH\"}", demoId);
                log(dev3, EntityType.TASK, t029.getId(), ActionType.STATUS_CHANGED, "{\"status\":\"TODO\"}",
                                "{\"status\":\"IN_PROGRESS\"}", demoId);

                // ECOM project logs
                Project ecom = projectRepository.findByProjectKey("ECOM").orElse(null);
                if (ecom != null && !activityLogRepository.findByProjectId(ecom.getId(), Pageable.ofSize(1))
                                .hasContent()) {
                        UUID ecomId = ecom.getId();
                        Task e016 = taskRepository.findByTaskCode("ECOM-016").orElse(null);
                        log(pm, EntityType.PROJECT, ecomId, ActionType.CREATED, null,
                                        "{\"name\":\"E-Commerce Platform\",\"key\":\"ECOM\"}", ecomId);
                        if (e016 != null) {
                                log(qa, EntityType.TASK, e016.getId(), ActionType.CREATED, null,
                                                "{\"taskCode\":\"ECOM-016\",\"type\":\"BUG\",\"priority\":\"CRITICAL\"}",
                                                ecomId);
                        }
                }
        }
