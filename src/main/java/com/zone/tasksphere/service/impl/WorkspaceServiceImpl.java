...
public class WorkspaceServiceImpl implements WorkspaceService {
        ...
        public List<WorkspaceResponse> getMyWorkspaces(UUID userId) {
                ...
                List<Workspace> workspaces = workspaceRepository.findAllByMemberUserId(userId).stream()
                                .sorted(Comparator
                                                ...
                                                .thenComparing(Workspace::getUpdatedAt,
        ...
        public WorkspaceResponse updateWorkspace(UUID workspaceId, UpdateWorkspaceRequest request, UUID requesterId) {
                ...
                if (request.getAvatarUrl() != null)
                        ws.setAvatarUrl(request.getAvatarUrl());
        ...
        public void deleteWorkspace(UUID workspaceId, UUID requesterId) {
                ...
                log.info("Workspace {} soft-deleted by {}, child projects moved to standalone", workspaceId,
                                requesterId);
        }
        ...
        public WorkspaceInviteResponse inviteMember(UUID workspaceId,
                ...
                workspaceInviteRepository
                                .findByWorkspaceIdAndInviteeEmailAndStatus(workspaceId, normalizedEmail,
                ...
                userOpt.ifPresent(invitee -> notificationService.createNotification(
                                invitee,
                                NotificationType.WORKSPACE_INVITED,
                                "Bạn nhận được lời mời vào workspace",
                ...
                log.info("Invitation email sent to {} (existingUser={}) for workspace {}", normalizedEmail,
                                userOpt.isPresent(), workspaceId);
        ...
        @Override
        @Transactional
        public Page<WorkspaceInviteListResponse> getInvitesByStatus(UUID workspaceId, UUID actorId, InviteStatus status,
                        Pageable pageable) {
                requireAdminOrOwner(getWorkspaceOrThrow(workspaceId), actorId);

                if (status == InviteStatus.PENDING) {
                        workspaceInviteRepository.markExpiredInvites(workspaceId, Instant.now(), InviteStatus.PENDING,
                ...
                return workspaceInviteRepository.findByWorkspaceIdAndStatus(workspaceId, status, pageable)
                                .map(invite -> WorkspaceInviteListResponse.builder()
                                                ...
                                                .daysLeft(invite.getStatus() == InviteStatus.PENDING
                                                                ? Math.max(0, ChronoUnit.DAYS.between(Instant.now(),
                                                                                invite.getExpiresAt()))
                                                                : null)
                                                .build());
        }
        ...
        public void resendInvite(UUID workspaceId, UUID inviteId, UUID actorId) {
                ...
                emailService.sendWorkspaceInviteEmail(
                                ...
                                invite.getInviteeUser() != null,
                                ws.getSlug(),
                                invite.getToken());
                if (invite.getInviteeUser() != null) {
                        notificationService.createNotification(
                                        invite.getInviteeUser(),
                                        NotificationType.WORKSPACE_INVITED,
                                        "Lời mời vào workspace đã được gửi lại",
        ...
        public void declineInvite(String token, UUID currentUserId) {
                ...
                afterCommit(() -> publishWorkspaceMemberEvent(invite.getWorkspace().getId(), "workspace.invite_updated",
                                Map.of(
                                                "inviteId", invite.getId(),
        ...
        public void acceptInviteAfterSignup(String token, User newUser) {
                ...
                if (workspaceMemberRepository.existsByIdWorkspaceIdAndIdUserId(invite.getWorkspace().getId(),
                                newUser.getId())) {
                        invite.setStatus(InviteStatus.ACCEPTED);
                        invite.setAcceptedAt(Instant.now());
                ...
                afterCommit(() -> publishWorkspaceMemberEvent(invite.getWorkspace().getId(), "workspace.member_added",
                                Map.of(
        ...
        public List<WorkspaceMemberResponse> getMembers(UUID workspaceId) {
                ...
                return members.stream()
                                .map(m -> toMemberResponse(m, m.getUser(),
                                                openTaskCounts.getOrDefault(m.getUser().getId(), 0)))
                                .collect(Collectors.toList());
        }
        ...
        private void requireAdminOrOwner(Workspace ws, UUID requesterId) {
                workspaceMemberRepository.findByWorkspaceIdAndUserId(ws.getId(), requesterId)
                                ...
                                .orElseThrow(() -> new Forbidden(
                                                "Cần quyền ADMIN hoặc OWNER để thực hiện thao tác này"));
        }
        ...
        private String resolveUniqueSlug(String base) {
                if (!workspaceRepository.existsBySlug(base))
                        return base;
                int suffix = 2;
                while (workspaceRepository.existsBySlug(base + "-" + suffix))
                        suffix++;
                return base + "-" + suffix;
        }
