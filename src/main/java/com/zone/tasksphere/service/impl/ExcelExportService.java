...
public class ExcelExportService {
    ...
    private static final String[] BURNDOWN_HEADERS = {
    ...
    public byte[] exportBurndownToExcel(List<Task> tasks, Project project, List<Sprint> sprints) {
        try (Workbook wb = new XSSFWorkbook();
             ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            Row headerRow = sheet.createRow(0);
            headerRow.setHeightInPoints(22);
            for (int i = 0; i < BURNDOWN_HEADERS.length; i++) {
                Cell cell = headerRow.createCell(i);
                cell.setCellValue(BURNDOWN_HEADERS[i]);
                cell.setCellStyle(headerStyle);
            }
            ...
            for (Task task : tasks) {
                ...
                createTextCell(row, 3, task.getAssignee() != null ? task.getAssignee().getFullName() : "Unassigned", textStyle);
                createTextCell(row, 4, task.getSprint() != null ? task.getSprint().getName() : "Backlog", textStyle);
                createTextCell(row, 5, task.getTaskStatus() != null ? task.getTaskStatus().name() : "", textStyle);
                createTextCell(row, 6, resolveProjectedDueDate(task, task.getSprint(), hoursPerStoryPoint), textStyle);
            }

            int[] columnWidths = { 4200, 14000, 4800, 7200, 7200, 4800, 4200 };
            for (int i = 0; i < BURNDOWN_HEADERS.length; i++) {
                sheet.setColumnWidth(i, columnWidths[i]);
            }
            ...
            for (Sprint sprint : orderedSprints) {
                ...
                Cell velocityCell = row.createCell(5);
                velocityCell.setCellValue(sprint.getVelocity() != null ? sprint.getVelocity() : 0);
                velocityCell.setCellStyle(textStyle);
                ...
                Cell countCell = row.createCell(6);
                countCell.setCellValue(sprintTaskCount);
                countCell.setCellStyle(textStyle);
            }
    ...
    private int computeProjectedDurationDays(Task task, double hoursPerStoryPoint) {
        double projectedHours = estimateHoursFromStoryPoints(task.getStoryPoints(), hoursPerStoryPoint);
        if (projectedHours > 0) {
